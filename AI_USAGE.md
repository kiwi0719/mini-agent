...
| 25 | （不是代码 bug，是认知偏差）此前把 Mock 用例 11"证据不足就拒绝猜测"当作 Agent 具备该能力的证据 | 真实模型跑同一任务时两个都编造了汇率 | 改写 DESIGN §8 限制 2 与 AI_USAGE，明确区分"框架支持这条路径"与"模型会选这条路径"；并指出要真正约束得在工具层面强制而非靠系统提示 |

...
20. **把溯源检查移出 Agent 循环**：候选人否掉方案 B（写入前拦截）——“太影响性能”——并要求把剩余缺口定性为模型能力问题，同时明确 RAG 与 Skills 不在 demo 范围内。AI 提出把同样的检查做成**离线 Trace 审计**（跑在任务之后，零运行开销、只报告不拦截），候选人采纳。这保留了溯源能力又绕开了性能顾虑。
21. **先测最便宜的方案再谈机制**：面对"模型编造汇率"，AI 给了 5 个方案。候选人判断"其实是模型能力弱的问题，做 A 测试一下吧，收益这么高"，要求先做零代码的提示方案并实测。结果证明这个判断是对的——提示修好了强模型，没修好弱模型，而失败原因（弱模型不做缺失检测）直接决定了下一步该做机械检测而不是继续加提示。
22. **Anthropic Provider 也要兼容非官方接口**：候选人要求 Anthropic 通道不只连官方 API，也要支持第三方兼容网关。AI 据此去掉官方 SDK 改为直接 fetch，并加 Kimi / GLM / DeepSeek / MiniMax 预设。
23. **交付前复核**：候选人重新贴出题目全部交付物要求，要求 AI 逐项复核哪些没完成，包括新增的设计。AI 据此发现远端缺失另一线程的全部文件并收口提交。
24. **用真实模型验证**：候选人把 OpenRouter key 放进 `.env`，要求 OpenAI 与 Anthropic 两种协议各测一个"性价比高、不要太贵太强"的模型，看实际运行效果。此前所有端到端结果都来自 Mock，这一步直接解除了文档里"最大限制"那一条。
...
| 26 | 审计工具第一版把所有工具调用参数都当作待查对象，23 份 Trace 误报 14 份，全是 `tail:100`、`window:60m` 这类查询参数 | 写完就在全部 31 份 Trace 上量噪声，而不是只测已知的编造案例 | 区分“数据”与“控制”：只审计 `calculator.expression` / `write_file.content` 这类承载事实内容的字段 |
| 27 | 第二版对**来源**也做结构性剥离，把 changelog 里 `## 3.9.3` 的版本号从来源池删掉，正文引用它反被误报（一次报出 24 个） | 同上，靠全量跑而不是单点验证发现 | 剥离只作用于被检查的文本，来源侧宁可多认 |
| 28 | AI 最初建议用"工具层面强制"（如要求先调 `get_exchange_rate`）来解决编造汇率 | 候选人判断"其实是模型能力弱的问题"，要求先测最便宜的方案 A（改提示） | A/B 实测证明候选人的判断更准：同一条提示规则修好了 haiku、对 gpt-4o-mini 完全无效，而且无效的原因是它从不检测"缺前提"，不是不听话。这个结论只有真跑 A/B 才能得到，直接上工具强制会掩盖掉"弱模型缺的是检测能力"这个事实 |
| 29 | K8s GPU 场景中重启类 Pod 没取 previous 容器日志，证据链缺失 | 跑示例时发现 | get_logs 对重启 Pod 取 previous 日志 |
| 30 | 只提交自己改的文件，导致远端长期缺少另一线程的全部工作，README 承诺的 21 个任务在远端并不存在 | 候选人要求复核交付物时逐项对照发现 | 收口提交全部文件，并从 GitHub 全新 clone 独立验证 |
| 31 | AI_USAGE 把 schema 离线校验写成"端到端验证" | 复核时自查发现 | 改为三档表格，区分端到端 / 仅协议层 / Mock 数据下的端到端 |
| 32 | 显式传入 `--api-key` 时，Anthropic Provider 构造函数仍把宿主机残留的失效 `ANTHROPIC_AUTH_TOKEN` 填进 Bearer 头 | 用真实 key 探测 OpenRouter 的 Anthropic 端点时鉴权异常 | 显式参数优先，不再回落到环境变量 |
| 33 | 两个会话各自给 `cli.ts` 加了 `auth-token` 选项，造成重复键 | 类型检查 / 读代码时发现 | 去重 |
...
- **提示词能修好强模型，修不好弱模型，而原因不是听话程度**。加了一条领域无关的数值来源规则后，claude-haiku-4.5 改为主动搜索、找不到就拒绝；gpt-4o-mini 仍然编造。关键证据在工具调用序列：haiku 多了一次 `search_text`，gpt-4o-mini 前后都没搜过。它不是读了规则却违反，而是从不进行"这里缺一个前提"这项检测——条件型规则对缺乏检测能力的模型没有附着点。由此得出一条可复用的结论：给弱模型写规则要尽量写成无条件动作，而不是依赖自我评估的条件分支。

...
| **Trace** (`src/trace.ts`) | 记录 User → Decision → ToolCall → ToolResult → … → Final；每步含 LLM 耗时、token usage，每个工具调用含 durationMs；输出 JSON + Markdown。它同时是**溯源记录**，`pnpm audit` 据此离线检查数值来源（§4.10） | |
...
## 4.10 离线 Trace 审计（数值溯源）

`pnpm audit <trace.json>`，见 `scripts/audit-trace.ts`。

**为什么放在循环外**：把溯源检查做进 Agent 循环（写入前拦截）代价不在 CPU——一次正则加集合比对可以忽略——而在**误报换来的模型往返**：每个被误判的千分位、日期、计数都要多一轮对话去澄清，正确任务反而被拖慢甚至带偏。放在任务结束后做，对运行性能零影响、不会误伤，代价是它只报告不阻止。它回答的是"这次运行的结论可不可信"，不是"要不要放行"。Trace 本来就记着每个值来自哪次工具调用，这些信息此前只用于展示。

**来源的定义**（整个工具的关键）：任务描述里的数字、以及**成功的工具结果**里的数字算来源；模型写出的**数据**（`calculator.expression`、`write_file.content`、最终答案）是待查对象。
把工具调用的参数当来源会漏掉最重要的情况：gpt-4o-mini 编汇率是把 `7.15` 塞进 calculator 表达式，于是结果里出现了一个"有来源"的总额，但那个总额的来源本身无来源。只看最终答案查不出来。

**只审计数据，不审计控制**：`tail: 100`、`window: "60m"`、`max_results: 500` 是查询参数，不是对世界的事实断言。第一版没分开，23 份 Trace 误报了 14 份，全是这类。现在用一张显式表声明每个工具的哪些字段承载事实内容。

**噪声处理**：抽数前剥掉 ISO 时间戳、Markdown 小节号、列表序号、IPv4；按绝对值比较（`-¥200.00` 里负号与数字被货币符号隔开）。剥离只作用于被检查的文本，**不作用于来源**——否则 changelog 里 `## 3.9.3` 这种标题版本号会从来源池消失，正文引用它反被误报（这个 bug 实测中出现过）。

**实测结果**（31 份 Trace）：

| 类别 | 数量 | 说明 |
|---|---|---|
| 真实编造 | 2 | gpt-4o-mini 的 `7.15`（表达式里的假汇率）与 `1316.75`（它自己四舍五入的总额） |
| 模型口算 | 5 | 比值、计数、单位换算未经 calculator，违反系统提示规则 3——属于真阳性 |
| 已知误报 | 2 | 举例用的数字（"例如 1 USD = 7.2 CNY"）、建议阈值（"内存使用率 > 90% 告警"）。二者都需要理解意图才能排除，机械规则做不到 |
...
2. **框架能表达正确决策，但不能让模型做出正确决策**——这是真实模型暴露出的最重要一条：
   - **会编造缺失的前提，且提示只能修好一半**。任务"把销售额换算成美元"在 workspace 里没有任何汇率。Mock 的剧本是搜索确认后明确拒绝（examples/11），但两个真实模型最初都直接编了一个汇率并把报告写了出来，都没搜过。
     于是加了系统提示规则 7（数值必须有来源，找不到就停下说明缺什么，领域无关、不给"标注假设后继续"的退路），做了一次 A/B：**`claude-haiku-4.5` 被修好了**——它改为先 `search_text` 找汇率、找不到才拒绝，且不写文件；**`gpt-4o-mini` 没有任何变化**，仍然编造、仍然写出报告。
     关键在工具调用序列：haiku 改提示后多了一次搜索，gpt-4o-mini 前后都是 `read_file → calculator → write_file`，从不搜索。**它不是读了规则却违反，是没意识到"缺前提"这个情况发生了**；给一条"发现缺前提时该怎么办"的规则，对一个从不做这项检测的模型没有作用。
     **结论是：这条剩余缺口属于模型能力，不属于框架缺陷，并且本项目刻意不再往下补。** 框架该提供的都提供了——搜索工具在、拒绝路径通（Mock 用例 11 走的就是这条）、提示里写明了规则、有能力的模型照做了。剩下的是"模型会不会自己意识到缺了一个前提"，这是模型侧的能力，不是再加一层框架代码能补上的。
     继续补的话有三条路，都不适合这个 demo：**写入时数值溯源检查**（方案 B）代价不在 CPU（一次正则加集合比对可以忽略），而在误报——报告里的千分位、日期、计数都会被当成无来源数值，每一次误报都要多一轮模型调用来澄清，正确任务反而被拖慢甚至带偏；**RAG / 外部数据源**（真去查汇率）超出"受限 workspace 内的 Agent"这个设定，且引入网络依赖与外部 key；**把领域规程写成 Skills**（换算类任务必须先取汇率）本质是把判断硬编码进流程，能解决汇率，解决不了下一个没预料到的缺失前提。
     所以文档里明确记下边界：**用 haiku 档及以上的模型时这条能力成立，用 gpt-4o-mini 档时不成立**。对照数据见 [examples/real-model/README.md](./examples/real-model/README.md#ab给系统提示加一条数值溯源规则)。

     一个更一般的结论，对写系统提示有直接用处：**"当你发现 X 时，做 Y"这类条件规则，只在模型具备"发现 X"的能力时才有效**。无条件规则（"算数一律用 calculator"）弱模型也能照做；依赖模型自我评估的条件规则（"不确定就停下"）在弱模型上等于没写。规则 3 和规则 7 的效果差异就是这条的实证。

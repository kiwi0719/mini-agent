...
2. **框架能表达正确决策，但不能让模型做出正确决策**——这是真实模型暴露出的最重要一条：
   - **会编造缺失的前提，且提示只能修好一半**。任务"把销售额换算成美元"在 workspace 里没有任何汇率。Mock 的剧本是搜索确认后明确拒绝（examples/11），但两个真实模型最初都直接编了一个汇率并把报告写了出来，都没搜过。
     ...
     所以正确的下一步是**机械检测而不是继续加提示**：写入前把内容里的数值与历史工具结果比对，对不上的警告或拒绝——检测由框架做，不依赖模型是否有这个意识。这一层尚未实现。对照数据见 [examples/real-model/README.md](./examples/real-model/README.md#ab给系统提示加一条数值溯源规则)。
